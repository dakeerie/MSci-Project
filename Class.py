...
# Use '1j' for complex number standard in Python, though complex(0,1) works too
j = 1j 
...
class SchwarzschildPerturbation:
    ...
    def solve(self, Psi0, Nt):
        ...
        # First step (Leapfrog start)
        psi_arr[1:-1, 1] = (psi_arr[1:-1, 0] + 0.5*(self.dt/self.dx)**2*(psi_arr[2:, 0] + 
                            psi_arr[:-2, 0] - 2*psi_arr[1:-1, 0]) 
                            - 0.5*self.V[1:-1]*psi_arr[1:-1, 0]*self.dt**2)
        ...
        # Outgoing boundary conditions (simple approx)
