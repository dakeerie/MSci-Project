...
parser.add_argument('--run_id', type = int, required = True, help = 'The HTCondor Process ID for repeat runs')
...
base_path = f'./RepeatRunQNM/l{mode}/delta{delta}/{ID}'
...
print('-'*30)
print(f'Starting training for l = {mode} with delta = {delta}')
...
N_points = 10000
learning_rate = 1e-3
model = Model(1, 2, 32, num_hidden_layers = 3, rho_init = rho_perturbed).to(device = device, dtype = DTYPE)
...
plt.plot(rho_real_array, rho_imag_array, 'r--', alpha = 1.0)
...
plt.xlabel(r'$\Re(\rho)$', fontsize = 25)
plt.ylabel(r'$\Im(\rho)$', fontsize = 25)
plt.title(rf'l = {mode}, \Delta \rho = {initial_dist}', fontsize = 25)
...
results = {
    ...
    'final_ode_residual': {
        'x': x_plot[idx],
        'res_re': plot_data['res_re'].flatten()[idx],
        'res_im': plot_data['res_im'].flatten()[idx],
        'w_re': plot_data['re_w'].flatten()[idx],
        'w_im': plot_data['im_w'].flatten()[idx]
    }
}
...
with open(result_path, 'w') as f:
    ...
    f.write(f"initial_rho_re={rho_perturbed.real:.8f}\n")
    f.write(f"initial_rho_im={rho_perturbed.imag:.8f}\n")
    f.write(f"rho_re={model.rho_re.item():.8f}\n")
    f.write(f"rho_im={model.rho_im.item():.8f}\n")
    f.write(f"omega_re={final_omega.real:.8f}\n")
    f.write(f"omega_im={final_omega.imag:.8f}\n")
    f.write(f"target_omega_re={omega.real:.8f}\n")
    f.write(f"target_omega_im={omega.imag:.8f}\n")
    f.write(f"final_distance={abs(complex(model.rho_re.item(), model.rho_im.item()) - rho):.8f}\n")

checkpoint_path = os.path.join(base_path, f'pinn_checkpoint_qnm_l{mode}_delta{delta}_run{ID}.pth') 
